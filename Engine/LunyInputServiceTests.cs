...
namespace Luny.Test.Engine
{
	...
	public sealed class LunyInputServiceTests
	{
		...
		[Test]
		public void GetButtonValue_Returns_Analog_Value()
		{
			_service.SimulateButtonInput("Trigger", true, 0.75f);

			Assert.That(_service.GetButtonValue("Trigger"), Is.EqualTo(0.75f));
		}

		[Test]
		public void GetButtonValue_Returns_Zero_After_Release()
		{
			_service.SimulateButtonInput("Trigger", true, 0.75f);
			_service.SimulateButtonInput("Trigger", false);

			Assert.That(_service.GetButtonValue("Trigger"), Is.EqualTo(0f));
		}
		...
		public void OnInputAction_Event_Fires_For_Axis()
		{
			...
			_service.SimulateAxisInput("Move", new LunyVector2(1f, 0f));

			Assert.That(received.ActionName, Is.EqualTo("Move"));
			Assert.That(received.ActionType, Is.EqualTo(LunyInputActionType.Axis));
			Assert.That(received.AxisValue, Is.EqualTo(new LunyVector2(1f, 0f)));
		}
